        ...
        <div class="col-md-8">
            <div class="row">
                <form class="navbar-form" action="" method="post" style="width: 100%" >
                    <input type="hidden" id="id_leftright" value="<?php if(isset($leftright->id)){ echo $leftright->id; } ?>"/>
                    <div class="form-group" style="width:100%;">
                        <div class="col-md-4">
                            <label for="" style="margin-top: 7px" class="pull-right">Tên content left-right</label>
                        </div>
                        <div class="col-md-8">
                            <input type="text" class="form-control" value="<?php if(isset($leftright->name)){ echo $leftright->name; }?>" name="name" id="name" style="width: 100%; border-radius: 0px"/>
                        </div>
                    </div>
                    ...
                    <div class="form-group" style="width:100%;  margin-top: 20px">
                        ...
                        <div class="col-md-8">
                            <textarea name="infor" class="form-control" style="width: 100%" id="infor" cols="30" rows="10"><?php if(isset($leftright->infor)){ echo $leftright->infor; }?></textarea>
                        </div>
                    </div>
                    <div class="clearfix"></div>
                    <div class="col-md-12">
                        <button class="btn btn-primary pull-right " onclick="Add_contentleft_ajax();" style="border-radius: 0px; margin-top: 20px;">Thêm mới</button>
                    </div>
                </form>
            </div>
        </div>
...
<script type="text/javascript">
    function Add_contentleft_ajax() {
        ...
        var id_leftright = $('#id_leftright').val();
        $.ajax({
            url: base_url() + 'website/Uet_content_leftright/Add_contentleft_ajax',
            type: "POST",
            data: {name: name, infor: infor, id_content: id_content, id_leftright: id_leftright},
            success: function (res) {
                if (res == 1) {
                    alert("Bạn đã cập nhật thành công");
                    window.location.href = base_url()+ "/website/Uet_content_leftright/";
                } else {
