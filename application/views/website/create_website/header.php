                        ...
                        <?php
                            if(isset($header) && !empty($header)){
                                foreach($header as $head){
                                    if($head->id_website == $web->id){ ?>
                                        <tr>
                                            <td style="width: 90%">
                                                <a onclick="select_header(<?=$head->id;?>)" style="cursor: pointer">
                                                    <img src="<?=base_url($head->url.'image.png')?>" style="width: 100%; height: 150px;" alt=""/>
                                                </a>
                                            </td>
                                        </tr>
                        <?php       }
...
<div class="modal fade" id="header" tabindex="-1" role="dialog">
        ...
        <div style="border-radius: 0px; margin-top: 150px;" class="modal-content">
            ...
            <div class="modal-body" style="border-radius: 0px;">
                <h4 style="text-transform: uppercase; color: black;">
                    Bạn chọn header bằng cách <br> click vào Hình ảnh hiển thị trên màn hình !
                </h4>
            </div>
...
<script type="text/javascript">
    ...
    function select_header(id){
        var check = confirm("Bạn có chắc chắn muốn chọn mẫu header này?");
        if(check == true){
            $.ajax({
                url: base_url() + 'website/Uet_createwebsite/select_header',
                type: "POST",
                data: {id: id},
